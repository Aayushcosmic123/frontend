...
const login = () => {
    ...
    const handleLogin = (e)=>{
        e.preventDefault();
        // backend here
        fetch('https://linkspectrum-backend.onrender.com/api/login', {
            method: 'POST',
            headers:{
        ...
        .then(data=>{
            if(data.status==='success'){
                ...
                localStorage.setItem('LinkSpectrumToken', data.token);
        ...
        <section className={styles.background + " min-h-screen flex justify-center items-center"}>
            <div className="main">
                ...
                <form onSubmit={handleLogin} className='flex flex-col gap-4 text-lg mt-5'>
                    ...
                    <input value={password} onChange={e=>setPassword(e.target.value)} className='px-3 py-2 rounded-md focus:outline-none' type="password" placeholder='Set a password' required/>
                   </span> 
