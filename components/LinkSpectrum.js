...
import LinkSpectrumCard from './LinkSpectrumCard'
import {AnimatePresence, motion} from 'framer-motion';

const LinkSpectrum = ({data}) => {
            ...
            <div className='flex flex-col justify-center max-w-7xl m-auto md:my-5 w-full md:w-2/5'>
                <AnimatePresence>
                    {links.map((link, index)=>(
                        ...
                        >
                            <LinkSpectrumCard title={link.title} url={link.url} image={link.icon}/>
                        </motion.div>
                    ))}
                </AnimatePresence>
            </div>
