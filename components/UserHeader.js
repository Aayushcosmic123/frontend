...
const UserHeader = () => {
    ...
    const handleLogout = ()=>{
        localStorage.removeItem('LinkSpectrumToken');
        router.push('/login');
    }
    ...
    return (
        ...
        <header className='flex flex-row justify-between items-center'>
            <div className="flex flex-col md:flex-row p-5">
                ...
                <button className='inline-flex w-full md:w-auto px-5 py-3 text-purple-700 font-bold hover:text-purple-700 hover:bg-purple-100 rounded-md mb-3 border-2 border-purple-500'>
                    <img src="/svg/url.svg" className='w-6 mr-3'/>
                    Edit Links
                </button>
                </Link>
                <Link href="/edit/profile">
                <button className='inline-flex w-full md:w-auto px-5 py-3 text-red-600 font-bold hover:text-red-700 hover:bg-red-100 rounded-md mb-3 border-2 border-red-500 md:ml-4'>
                    <img src="/svg/user.svg" className='w-6 mr-3'/>
                    Edit profile
                </button>
                </Link>
            </div>
            <Link href={`https://frontend-linkspectrum.vercel.app/${handle}`}>
